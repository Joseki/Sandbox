...
namespace MyApplication\LeanMapper\DI;

use MyApplication\LeanMapper\Mapper;
use Nette;
use Tracy\Debugger;
...
class Extension extends Nette\DI\CompilerExtension
{
    ...
    public function loadConfiguration()
    {
        ...
        $container->addDefinition($this->prefix('mapper'))
            ->setClass('MyApplication\LeanMapper\Mapper', array($packages, $tables));
        ...
        if ($useProfiler) {
            ...
            if (is_array($useProfiler) && isset($useProfiler['file'])) {
                $fileLogger = $container->addDefinition($this->prefix('fileLogger'))
                    ->setClass('MyApplication\LeanMapper\FileLogger');
